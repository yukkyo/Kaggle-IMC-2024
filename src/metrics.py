...
def evaluate_rec(
    gt_df, user_df, inl_cf=0.8, strict_cf=0.5, skip_top_thresholds=2, to_dec=3, thresholds=None
):
...
def score(solution: pd.DataFrame, submission: pd.DataFrame) -> float:
    ...
    for dataset in scenes:
        ...
        runtime_tmp = time.time() - start

        # for view
        dict_df["dataset"].append(dataset)
        dict_df["len_gt"].append(len(gt_ds))
        dict_df["len_pred"].append(len(user_ds))
        dict_df["runtime"].append(runtime_tmp)
        dict_df["maa"].append(result)
